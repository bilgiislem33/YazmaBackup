...
foreach($x in @('ContinuityScore','DisasterSimulationStep','BusinessKey','InferCriticality','MissingRecoveryCoverage','RtoTargetMinutes','Recovery Plan kapsamı yok')){
...
foreach($x in @('Business Continuity Command Center','Continuity Score','Kurtarma Sırası','Disaster Simulation','sessizce başlatmaz')){
 if(-not $ui.Contains($x)){throw ('R23 UI invariant eksik: '+$x)}
}
Write-Host 'PASS: R23 Business Continuity Command Center statik kalite kapısı.'
