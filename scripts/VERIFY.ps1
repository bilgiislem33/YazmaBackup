...
try {
    ...
    $agentCircuitWorker=Get-Content .\src\YazmaBackup.Agent\AgentWorker.cs -Raw
    foreach($required in @('CanProbeOpenRepositoryCircuit(command.Type)','string.Equals(commandType, "TestNasAccess"','_repositoryCircuit.ReportSuccess(repositoryId)')){if($agentCircuitWorker -notmatch [Regex]::Escape($required)){throw "Repository recovery probe invariant eksik: $required"}}
