...
foreach($x in @('Read Path','normalizedReadPath','R14 production active-active')){
  if(-not $ui.Contains($x)){throw ('R14 UI invariant eksik: '+$x)}
}
Write-Host 'PASS: R14 Direct SQL Read Cutover statik kalite kapısı.'
