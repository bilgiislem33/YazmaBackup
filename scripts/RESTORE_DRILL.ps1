...
$body = @{ repositoryRoot = $RepositoryRoot; repositoryId = $RepositoryId; sourceRoot = $SourceRoot } | ConvertTo-Json
$queued = Invoke-RestMethod -Method Post -Uri "$base/api/v1/admin/agents/$AgentId/restore-drill" -Headers $headers -ContentType 'application/json' -Body $body
Write-Host "Restore drill kuyruğa alındı: $($queued.commandId)"
for ($i = 0; $i -lt $MaxPolls; $i++) {
    $result = Invoke-RestMethod -Method Get -Uri "$base/api/v1/admin/commands/$($queued.commandId)" -Headers $headers
    if ($result.completed) {
        if (-not $result.succeeded) { throw "Restore drill başarısız: $($result.error)" }
