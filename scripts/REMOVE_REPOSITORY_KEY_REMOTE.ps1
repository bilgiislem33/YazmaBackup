...
$body = @{ repositoryRoot = $RepositoryRoot; repositoryId = $RepositoryId; keyId = $KeyId } | ConvertTo-Json
$queued = Invoke-RestMethod -Method Post -Uri "$base/api/v1/admin/agents/$AgentId/repository-key/remove" -Headers $headers -ContentType 'application/json' -Body $body
Write-Host "Repository key remove komutu kuyruğa alındı: $($queued.commandId)"
for ($i = 0; $i -lt $MaxPolls; $i++) {
    $result = Invoke-RestMethod -Method Get -Uri "$base/api/v1/admin/commands/$($queued.commandId)" -Headers $headers
    if ($result.completed) {
        if (-not $result.succeeded) { throw "Repository key remove başarısız: $($result.error)" }
